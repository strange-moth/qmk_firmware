...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [_FN3] = LAYOUT_ansi_67(
        ...
        KC_CAPS,          KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS,           KC_TRNS, KC_PGUP,
        KC_TRNS, KC_TRNS, KC_TRNS,                            KC_TRNS,                            KC_TRNS, KC_TRNS,  KC_TRNS, KC_HOME, KC_PGDN, KC_END)
};
...
void rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (host_keyboard_led_state().caps_lock) {
        uint8_t shift_left = 44;
        if (led_min <= shift_left && led_max >= shift_left) {
	    rgb_matrix_set_color(shift_left, RGB_RED);
        };
        uint8_t shift_right = 55;
        if (led_min <= shift_right && led_max >= shift_right) {
	    rgb_matrix_set_color(shift_right, RGB_RED);
        };
